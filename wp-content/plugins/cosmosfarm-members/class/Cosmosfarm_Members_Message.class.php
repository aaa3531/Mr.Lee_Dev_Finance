				...
				if($this->post_id){
					...
					if($this->is_user_subnotify){
						...
						if($this->is_user_subnotify_sms($this->user_id)){
							...
							if($phone){
								if($option->alimtalk_service && $option->messages_subnotify_alimtalk_template){
									$template_code = $option->messages_subnotify_alimtalk_template;
									
									$alimtalk_args = array();
									$alimtalk_args['to'] = $phone;
									$alimtalk_args['template_code'] = $template_code;
									$alimtalk_args = apply_filters('cosmosfarm_members_messages_subnotify_alimtalk_args', $alimtalk_args, $this);
									cosmosfarm_members_alimtalk_send($phone, $template_code);
								}
								else if($option->sms_service && !$option->messages_subnotify_alimtalk_template){
									$sms_args = array();
									$sms_args['to'] = $phone;
