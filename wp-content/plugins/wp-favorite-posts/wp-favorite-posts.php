...
function wpfp_link( $return = 0, $action = "", $show_span = 1, $args = array() ) {
    ...
    $str = "";
    if ($show_span)
        $str = "<span class='wpfp-span add-my-news'>";
//    $str .= wpfp_before_link_img();
//    $str .= wpfp_loading_img();
    if ($action == "remove"):
        $str .= wpfp_link_html($post_id, "- 구독 취소하기", "remove");
    elseif ($action == "add"):
        $str .= wpfp_link_html($post_id, "+ 기사 구독하기", "add");
...
function wpfp_link_html($post_id, $opt, $action) {
    $link = "<a class='wpfp-link ".$action."' href='?wpfpaction=".$action."&amp;postid=". esc_attr($post_id) . "' title='". $opt ."' rel='nofollow'>". $opt ."</a>";
...
function wpfp_init() {
    ...
    $wpfp_options['added'] = "가사가 구독되었습니다.";
    $wpfp_options['remove_favorite'] = "Remove from favorites";
    $wpfp_options['removed'] = "구독이 취소되었습니다.";
...
function wpfp_remove_favorite_link($post_id) {
    if (wpfp_is_user_can_edit()) {
        ...
        $link = "<a id='rem_$post_id' class='$class' href='?wpfpaction=remove&amp;page=1&amp;postid=". $post_id ."' title='".wpfp_get_option('rem')."' rel='nofollow'>구독 취소하기</a>";
...
function wpfp_clear_list_link() {
    if (wpfp_is_user_can_edit()) {
        $wpfp_options = wpfp_get_options();
        echo "<div id='remove-all-fav'><a class='wpfp-link remove-all' href='?wpfpaction=clear' rel='nofollow'>구독 전체 취소하기</a></div>";
    }
}
